% bar_multSize_sameSeq_PTB_25mm.m
%
%
% TCS 6/10/2017
%
...
%
% TCS 9/18/2017 - built for 2.5 mm sequence (750 ms TR), 120 HZ!!!!!
%
% 346.5 s - 462 TRs at 0.75 s TR



function bar_multSize_sameSeq_PTB_25mm(subj,run,seq)
...
if p.scanner == 0
    ...
    p.refresh_rate = 120;
    p.screen_height = 34.4; % cm prior to 9/14/2017 - was 33 cm, adjusted, now 36 cm tall; adjusted for 1024 pix height
...
if p.scanner == 1
    p.start_wait = 11.25; % s - time after trigger
    p.end_wait   = 11.25; % s - time after last bar sweep (2.4 s x 4)
else
    p.start_wait = 1;
    p.end_wait = 1;
end

if p.refresh_rate == 120
    p.dot_life = p.dot_life*2;
end
